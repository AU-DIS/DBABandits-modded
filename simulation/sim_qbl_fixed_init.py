...
class Simulator(BaseSimulator):
    ...
    def run(self,exp_report_list, version, exp_id_list) -> None: #TODO: correct return type
        ...
        init = lambda: None
        init.id = -1
        init.predicates = all_columns
        init.payload = {}
        init.selectivity = defaultdict(lambda: 1)
        ...
        for key, index_arm in bandit_arms_tmp.items():
            ...
            #    max(init.table_scan_times[index_arm.table_name])
            #    if init.table_scan_times[index_arm.table_name]
            #    else 0
            #)
